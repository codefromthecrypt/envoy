...
// Verifies HTTP transport with resource_detectors creates a valid access log instance.
TEST_F(OpenTelemetryAccessLogConfigTest, HttpConfigWithResourceDetectorsOk) {
  envoy::extensions::access_loggers::open_telemetry::v3::OpenTelemetryAccessLogConfig config;
  auto* http_service = config.mutable_http_service();
  http_service->mutable_http_uri()->set_uri("http://localhost:4318/v1/logs");
  http_service->mutable_http_uri()->set_cluster("otel_collector");
...
// Verifies gRPC transport with resource_detectors creates a valid access log instance.
TEST_F(OpenTelemetryAccessLogConfigTest, GrpcConfigWithResourceDetectorsOk) {
  EXPECT_CALL(context_.server_factory_context_.cluster_manager_.async_client_manager_,
              factoryForGrpcService(_, _, _))
      .WillOnce(Invoke([](const envoy::config::core::v3::GrpcService&, Stats::Scope&, bool) {
        return std::make_unique<NiceMock<Grpc::MockAsyncClientFactory>>();
