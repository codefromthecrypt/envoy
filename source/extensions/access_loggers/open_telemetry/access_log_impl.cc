...
AccessLog::AccessLog(
  ...
  tls_slot_->set([this, config, detected_resource](Event::Dispatcher&) {
    // Set detected_resource in thread-local storage before creating logger.
    // Use dynamic_cast to handle the case where cache is a mock in tests.
    auto* concrete_cache = dynamic_cast<GrpcAccessLoggerCacheImpl*>(access_logger_cache_.get());
    if (concrete_cache != nullptr) {
      concrete_cache->setDetectedResource(detected_resource);
    }
