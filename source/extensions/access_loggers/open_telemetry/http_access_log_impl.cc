...
HttpAccessLoggerImpl::HttpAccessLoggerImpl(
    ...
    : cluster_manager_(cluster_manager), http_service_(http_service),
      buffer_flush_interval_(DefaultBufferFlushInterval),
      max_buffer_size_bytes_(DefaultMaxBufferSizeBytes) {
...
void HttpAccessLoggerImpl::initMessageRoot(
    const envoy::extensions::access_loggers::open_telemetry::v3::OpenTelemetryAccessLogConfig&
        config,
...
HttpAccessLog::HttpAccessLog(
  ...
  tls_slot_->set([this, config, detected_resource](Event::Dispatcher&) {
    return std::make_shared<ThreadLocalLogger>(
        access_logger_cache_->getOrCreateLogger(config, http_service_, detected_resource));
  });
